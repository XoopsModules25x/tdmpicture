<?php
// $Id: pdf_paper.php 8168 2011-11-06 23:12:08Z beckmi $
//  ------------------------------------------------------------------------ //
...
include_once XOOPS_ROOT_PATH.'/modules/TDMAssoc/class/tdmassoc_pdf_table.php';
include_once XOOPS_ROOT_PATH.'/modules/TDMAssoc/admin/functions.php';

global $xoopsDB, $xoopsConfig, $xoopsModuleConfig;

  if( file_exists(XOOPS_ROOT_PATH."/modules/TDMAssoc/language/".$xoopsConfig['language']."/admin.php") ) {
	 include_once(XOOPS_ROOT_PATH."/modules/TDMAssoc/language/".$xoopsConfig['language']."/admin.php");
  } else {
	 include_once(XOOPS_ROOT_PATH."/modules/TDMAssoc/language/english/admin.php");
  }
...
switch( $option )
{
   ...
   break;
   	...
   	case "list_office_and_admin":
		...
		$pdf->Table('SELECT M.member_name, M.member_firstname, M.member_adress, M.member_zipcode, M.member_town, M.member_phone, S.status_name FROM '.$xoopsDB->prefix("tdmassoc_member").' M, '.$xoopsDB->prefix("tdmassoc_status").' S WHERE S.status_id=M.member_status AND M.member_office = "1" ORDER BY S.status_order limit 0,10',$prop);
		...
		$pdf->Table('SELECT M.member_name, M.member_firstname, M.member_adress, M.member_zipcode, M.member_town, M.member_phone, S.status_name FROM '.$xoopsDB->prefix("tdmassoc_member").' M, '.$xoopsDB->prefix("tdmassoc_status").' S WHERE S.status_id=M.member_status AND M.member_office = "0" ORDER BY S.status_order limit 0,10',$prop);
		$pdf->Output();
	...
	case "list_admin":
		...
		$pdf->Table('SELECT M.member_name, M.member_firstname, M.member_adress, M.member_zipcode, M.member_town, M.member_phone, S.status_name FROM '.$xoopsDB->prefix("tdmassoc_member").' M, '.$xoopsDB->prefix("tdmassoc_status").' S WHERE S.status_id=M.member_status ORDER BY S.status_order limit 0,10',$prop);
		$pdf->Output();
		
	break;
	
	case "ticket":
		$ticketHandler =& xoops_getModuleHandler('tdmassoc_ticket', 'TDMAssoc');
		...
		$text_color = TDMAssoc_color($ticket->getVar('ticket_text_color'));
					...
					{	
						if ( ( 297 - $ticket_height ) > $ticket_height_fixe )
						{
							...
							{	
								...
								{
									$num_ticket++;
									$pdf->Image(''.XOOPS_ROOT_PATH.'/uploads/TDMAssoc/images/ticket/'.$ticket_picture.'', $ticket_width, $ticket_height, $ticket_width_fixe, $ticket_height_fixe);
...
$newsletter_handler =& xoops_getModuleHandler('tdmassoc_newsletter', 'TDMAssoc');
$newsletter =& $newsletter_handler->get($_REQUEST['newsletter_id']);
...
$color = TDMAssoc_color($newsletter->getVar("newsletter_color"));
$pdf=new FPDF();
$pdf->AddPage();
//titre
...
//pour les produits
	...
	$product_handler =& xoops_getModuleHandler('tdmassoc_product', 'TDMAssoc');
	$stock_handler =& xoops_getModuleHandler('tdmassoc_stock', 'TDMAssoc');
	$product =& $product_handler->get($_REQUEST['product_id']);
	...
	$cat = array('1' =>_AM_TDMASSOC_PRODUCTCAT_ACHAT, '2' => _AM_TDMASSOC_PRODUCTCAT_VENTE, '3' => _AM_TDMASSOC_PRODUCTCAT_LOCATION, '4' => _AM_TDMASSOC_PRODUCTCAT_PRETS , '5' => _AM_TDMASSOC_PRODUCTCAT_CADEAUX, '6' => _AM_TDMASSOC_PRODUCTCAT_DIVERS );

	//calcul les stocks	
	$criteria = new CriteriaCompo();
	$criteria->add(new Criteria('stock_product', $_REQUEST['product_id']));
	$stock_arr =& $stock_handler->getall($criteria);
	$stock_qte = 0;
	foreach (array_keys($stock_arr) as $s) {
	$math = $stock_arr[$s]->getVar('stock_math');
	$qte = intval($stock_arr[$s]->getVar('stock_qte'));
...
//pour les stocks
	...
	$stock_handler =& xoops_getModuleHandler('tdmassoc_stock', 'TDMAssoc');
	$product_handler =& xoops_getModuleHandler('tdmassoc_product', 'TDMAssoc');
	$stock =& $stock_handler->get($_REQUEST['stock_id']);
	$product =& $product_handler->get($stock->getVar('stock_product'));
