...
$xoopsOption['template_main'] = 'tdmpicture_viewcat.html';
...
//variable post
 ...
 //$limit = !empty($_REQUEST['limit']) ? $_REQUEST['limit'] : $xoopsModuleConfig['tdmpicture_page'];
...
$xoopsTpl->assign('limit', $xoopsModuleConfig['tdmpicture_page']);
...
//moyen des votes
	...
	$xoopsTpl->append('file', $file);
		...
		//navigation
	if ( $numfile > $limit ) {
	$pagenav = new XoopsPageNav($numfile, $xoopsModuleConfig['tdmpicture_page'], $start, 'start', 'ct='.$ct.'&tris='.$tris.'&limit='.$limit);
...
$xoTheme->addMeta( 'meta', 'keywords', tdmpicture_keywords($meta_description));
