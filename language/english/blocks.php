<?php

/**
 * ****************************************************************************
 *  - TDMPicture By TDM   - TEAM DEV MODULE FOR XOOPS
 *  - GNU Licence Copyright (c)  (http://www.)
 *
 * La licence GNU GPL, garanti à l'utilisateur les droits suivants
 *
 * 1. La liberté d'exécuter le logiciel, pour n'importe quel usage,
 * 2. La liberté de l' étudier et de l'adapter à ses besoins,
 * 3. La liberté de redistribuer des copies,
 * 4. La liberté d'améliorer et de rendre publiques les modifications afin
 * que l'ensemble de la communauté en bénéficie.
 *
 * @copyright           (http://www.)
 * @license         http://www.fsf.org/copyleft/gpl.html GNU public license
 * @author          TDM ; TEAM DEV MODULE
 *
 * ****************************************************************************
 */

define('_MB_TDMPICTURE_DISPLAY', 'Display');
define('_MB_TDMPICTURE_TITLELENGTH', 'Full length');
define('_MB_TDMPICTURE_CATTODISPLAY', 'Select the categories displayed');
define('_MB_TDMPICTURE_ALLCAT', 'All Categories');

define('_MB_TDMPICTURE_SLIDELENGTH', 'Number of images in the loop only for Animation < View');
define('_MB_TDMPICTURE_MINTHUMBWIDTH', 'Max thumbnail width');
define('_MB_TDMPICTURE_MINTHUMBHEIGTH', 'Min thumbnail height');

define('_MB_TDMPICTURE_FILEVOTE', 'Votes');
define('_MB_TDMPICTURE_FILECOUNTS', 'Popularity');
define('_MB_TDMPICTURE_FILEHITS', 'Hits');

define('_MB_TDMPICTURE_MORE', 'About');
define('_MB_TDMPICTURE_SCREEN', 'Full Screen');
define('_MB_TDMPICTURE_POWERBY', 'Submitted by');
define('_MB_TDMPICTURE_IN', 'the');

//
define('_MB_TDMPICTURE_SELECT_STYLE', 'Block Style');
define('_MB_TDMPICTURE_SELECT_TEXT', 'Text');
define('_MB_TDMPICTURE_SELECT_IMAGE', 'Image');
define('_MB_TDMPICTURE_SELECT_IMAGE_DESC', 'Image/Info');
define('_MB_TDMPICTURE_SELECT_SLIDE', 'Animation');
define('_MB_TDMPICTURE_SELECT_MUR', 'Wall');
