...
class TDMObjectTree extends XoopsObjectTree
{
    ...
    public function _makeCatBoxOptions(
        ...
        if ($key > 0
            ...
            if (file_exists($imgpath) && $this->tree[$key]['obj']->getVar('cat_img') != 'blank.png') {
                $picture = '<a href ="' . $cat_link . '" title="' . $this->tree[$key]['obj']->getVar('cat_title') . '"><img src="' . TDMPICTURE_CAT_URL . $this->tree[$key]['obj']->getVar('cat_img') . '" class="img" width="'
                           . $helper->getConfig('tdmpicture_cat_width') . '"  height="' . $helper->getConfig('tdmpicture_cat_height') . '"></a>';
            } else {
                $picture = '<a href ="' . $cat_link . '" title="' . $this->tree[$key]['obj']->getVar('cat_title') . '"><img src="' . TDMPICTURE_CAT_URL . 'no_picture.png" class="img" width="' . $helper->getConfig('tdmpicture_cat_width')
                           . '"  height="' . $helper->getConfig('tdmpicture_cat_height') . '"></a>';
            }
            ...
            if (isset($selected) && $value == $selected) {
                ...
                if (isset($this->tree[$this->tree[$key]['parent']]['obj'])) {
                    $parent_link = TDMPICTURE_URL . '/viewcat.php?ct=' . $this->tree[$this->tree[$key]['parent']]['obj']->getVar('cat_id') . '&tris=' . $tris . '&limit=' . $limit;
                    $GLOBALS['navbar'] .= '<a href ="' . $parent_link . '" title="' . $this->tree[$this->tree[$key]['parent']]['obj']->getVar('cat_title') . '">' . $this->tree[$this->tree[$key]['parent']]['obj']->getVar('cat_title') . '</a> > ';
                }

                //$GLOBALS['cat_count'] = $count;
                $GLOBALS['cat_title'] = $this->tree[$key]['obj']->getVar('cat_title');

                $select = $this->makeSelBox('cat_pid', 'cat_title', '-', $selected, '', $this->tree[$key]['obj']->getVar('cat_id'), "OnChange='window.document.location=this.options[this.selectedIndex].value;'", 'tdmpicture_catview');
                $ret2   = '<li class="' . $GLOBALS['class'] . '"><div><div id="img">' . $picture . '<br><span id="tree_num"> (' . $count . ')</span></div><div id="tree_detail"><h2><a href ="' . $cat_link . '" title="'
                          . $this->tree[$key]['obj']->getVar('cat_title') . '">' . $this->tree[$key]['obj']->getVar($fieldName) . '</h2></a><span id="tree_text">' . $this->tree[$key]['obj']->getVar('cat_text')
                          . '</span></div><br style="clear: both;" /><div id="tree_form">' . $GLOBALS['navbar'] . $select . ' | ' . _MD_TDMPICTURE_TRIBY . ' > ' . $navtrie . '</div></div></div></li>';
            }

            //if ((!$prefix_curr) && ($this->tree[$key]['obj']->getVar('cat_pid') == $selected)) {
            if ($scat_display && $this->tree[$key]['obj']->getVar('cat_pid') == $selected) {
                if ((!$prefix_curr) || $helper->getConfig('tdmpicture_cat_display')) {
                    if (!empty($this->tree[$key]['child']) && $helper->getConfig('tdmpicture_cat_select')) {
                        $select = $this->makeSelBox('cat_pid', 'cat_title', '-', 0, '', $this->tree[$key]['obj']->getVar('cat_id'), "OnChange='window.document.location=this.options[this.selectedIndex].value;'", 'tdmpicture_catview');
                    } else {
                        $select = false;
                    }
                    ...
                    $ret .= '<li style="width:46%;" class="' . $GLOBALS['class'] . '"><div><div id="img">' . $picture . '<br><span id="tree_num"> (' . $count . ')</span></div><div id="tree_detail"><h2><a href ="' . $cat_link . '" title="'
                            . $this->tree[$key]['obj']->getVar('cat_title') . '">' . $cat_title . '</h2></a><span id="tree_text">' . $cat_text . '</span></div><br style="clear: both;" /><div id="tree_form">' . $select . '</div></div></div></li>';
                }
            }
     ...
     * @param string $prefix_curr
     */
    public function _makeSelBoxOptions2($fieldName, $selected, $key, &$ret, $perm, $prefix_orig, $prefix_curr = '')
    {
        global $start, $tris, $limit, $groups, $xoopsUser, $xoopsModule;
